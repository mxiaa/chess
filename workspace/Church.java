...
for(int x = -1; x <= 1; x++){
if(col + x >= 0 && col + x <= 7 && row + i >= 0 && row + i <= 7 && !
board[row + i][col + x].isOccupied() ){
moves.add (board[row + i ][col + x]);
}
else if(col + x + x >= 0 && col + x + x <= 7 && row + i +i >= 0 &&
row + i + i <= 7 && board[row + i][col + x].getOccupyingPiece().getColor()!= color
&& !board[row +i +i][col + x + x].isOccupied()){
