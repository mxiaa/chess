...
// Pre: Gets a square array array that is not null and a square which is not
//null
// Post: Gives an array of all available controlled squares for the piece at
//start
@Override
...
// Logic: Can move diagonal one square towards the opposite side of the board
//from
// where piece started and can capture pieces of opposite color to self if they
// are placed one square in front of piece towards the opposite side of board
//from
// where piece started. Can not move if another piece of any color or the edge
//of
...
int col = start.getCol();
// Finds if there is a square available for moving in move spot and then adds
//to ans array
...
// Finds if there is a piece available for capture in capture spot and then
//adds to ans array
if ((a[row][col].isOccupied() != false && a[row]
[col].getOccupyingPiece().getColor() != super.getColor()))
{
ans.add (a[row][col]);
}
return ans;
